...
void driving(float x, float y) {
    ...
    //SectionType currentSection = static_cast<SectionType>(99);
    ...
    if (currentSection == SectionType::DELIVERY_A || currentSection == SectionType::TUNNEL || currentSection == SectionType::PARKING_MISSION)
    {
        aspeed(target_vel, encoder_speed, CONTROL_PERIOD);
        cout << "Encoder Speed Section, Encoder Speed : " << encoder_speed << endl;
    }
    else
    {
        aspeed(target_vel, speed, CONTROL_PERIOD);
        cout << "Gps Speed Section, Gps Speed : " << speed << endl;
    }
}
...
int read_rddf() {
    ...
    string filename(ros::package::getPath("stier") + "/paths/2025-2-1_18-39_test-walk.txt");  // semi
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok()){
        ...
        */
        ...
        cout<<"cur_idx="<<cur_idx<<" tar_idx="<<target_idx<<" end_idx="<<cnt<<" Deg="<<drive_msg.Deg<<endl;
